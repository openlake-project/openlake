...
#[derive(Parser)]
#[command(about = "openlake rust layer 1 storage CLI")]
struct Cli {
    /// Root directory for on disk data.
    #[arg(long, default_value = "/tmp/openlake")]
    root: PathBuf,
...
async fn main() -> Result<()> {
    ...
    // dsync client wired in `openlaked` (see openlake_server::main).
    ...
    let dsync_by_set: Vec<Rc<openlake_storage::DsyncClient>> = (0..num_sets)
        .map(|_| Rc::new(openlake_storage::DsyncClient::no_op()))
        .collect();
