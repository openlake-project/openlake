...
async fn send_json<T: serde::Serialize>(s: &mut TcpStream, msg: &T) -> Result<()> {
    ...
    let r = s.write_all(len_v).await; r.0.context("write handshake len")?;
    let r = s.write_all(body ).await; r.0.context("write handshake body")?;
    Ok(())
}
...
pub async fn serve_target(bind: SocketAddr, buf_size: usize) -> Result<()> {
    ...
    let (setup, my_ep) = RdmaNode::start_local(&cfg)
        .map_err(|e| anyhow::anyhow!("rdma start_local: {e}"))?;
    let buf = setup.bulk_pool.acquire().await
    ...
    loop {
        ...
        match recv_json::<BenchHandshake>(&mut stream).await {
            Ok(client_msg) => {
                tracing::info!(client_dct = client_msg.ep.dct_num,
...
fn client_routing_table_unused() -> u32 { 0 }
...
pub async fn run_client(args: &ClientArgs, block_bytes: &[u64]) -> Result<Report> {
    ...
    let scratch_size = (max_block as usize)
        .checked_mul(max_batch as usize).context("buf overflow")?
        .checked_mul(max_threads as usize).context("buf overflow")?;

    let cfg = cfg_for(SELF_CLIENT_ID, scratch_size.max(1 << 20));
    let (setup, my_ep) = RdmaNode::start_local(&cfg)
        .map_err(|e| anyhow::anyhow!("rdma start_local: {e}"))?;
    let local_buf = setup.bulk_pool.acquire().await
        .map_err(|e| anyhow::anyhow!("bulk_pool acquire: {e}"))?;
    ...
    let peer = node.peer(SELF_TARGET_ID)
        .ok_or_else(|| anyhow::anyhow!("peer absent from routing table"))?;
    let ah = node.ah_cache.get_or_create(peer)
    ...
    let op_str: &'static str = match args.op { OpArg::Read => "read", OpArg::Write => "write" };
    ...
    for &threads in &args.threads {
        for &batch in &args.batch_sizes {
            for &block in block_bytes {
                let cell = run_cell(
                    &node, ah, peer_dct, peer_dck,
                    local_buf.addr(), local_buf.lkey(),
                    target_vaddr, target_rkey,
                    block, batch, threads, args.op, op_str,
                    warmup, duration,
                ).await?;
                cells.push(cell);
            }
...
async fn run_cell(
    ...
    let hist  = Arc::new(Mutex::new(Histogram::<u64>::new_with_bounds(1, 60_000_000, 3)?));
    let iters = Arc::new(std::sync::atomic::AtomicU64::new(0));
    let bytes = Arc::new(std::sync::atomic::AtomicU64::new(0));

    if !warmup.is_zero() {
        drive(node, ah, peer_dct, peer_dck,
              local_addr, local_lkey, target_vaddr, target_rkey,
              block, batch, threads, op, warmup,
              None, None, None).await?;
    }
    let started = Instant::now();
    drive(node, ah, peer_dct, peer_dck,
          local_addr, local_lkey, target_vaddr, target_rkey,
          block, batch, threads, op, duration,
          Some(hist.clone()), Some(iters.clone()), Some(bytes.clone())).await?;
...
async fn drive(
    ...
    for t in 0..threads {
        ...
        tasks.push(compio::runtime::spawn(async move {
            worker(
                sock, ah, peer_dct, peer_dck,
                local_addr + thread_local_off, local_lkey,
                target_vaddr + thread_remote_off, target_rkey,
                block, batch, op, deadline,
                hist, iters, bytes,
            ).await
        }));
    }
    for t in tasks {
        t.await.map_err(|e| anyhow::anyhow!("worker join: {e:?}"))??;
    }
    Ok(())
}
...
async fn worker(
    ...
    while Instant::now() < deadline {
        ...
        for i in 0..batch {
            ...
            let fut = async move {
                match op {
                    OpArg::Write => sock.rdma_write(
                        local, block as u32, local_lkey,
                        remote, target_rkey,
                        ah, peer_dct, peer_dck,
                    ).await,
                    OpArg::Read => sock.rdma_read(
                        local, block as u32, local_lkey,
                        remote, target_rkey,
                        ah, peer_dct, peer_dck,
                    ).await,
        ...
        for r in results { r.map_err(|e| anyhow::anyhow!("rdma op: {e}"))?; }
        if let Some(h) = &hist {
            let _ = h.lock().unwrap().record(lat_us.max(1));
        }
        if let Some(c) = &iters { c.fetch_add(1, std::sync::atomic::Ordering::Relaxed); }
        if let Some(c) = &bytes { c.fetch_add(block * batch as u64, std::sync::atomic::Ordering::Relaxed); }
    }
    Ok(())
}
...
fn print_target_banner(bind: SocketAddr, buf_size: usize, ep: &LocalEndpoint) {
    ...
    println!("Local endpoint: runtime_id=0 lid={} dct_num=0x{:x} gid={gid_hex}", ep.lid, ep.dct_num);
