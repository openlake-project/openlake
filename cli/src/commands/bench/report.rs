...
fn fmt_block(n: u64) -> String {
    ...
    const M: u64 = 1u64 << 20;
    const G: u64 = 1u64 << 30;
    if      n >= G && n % G == 0 { format!("{} GiB", n / G) }
    else if n >= M && n % M == 0 { format!("{} MiB", n / M) }
    else if n >= K && n % K == 0 { format!("{} KiB", n / K) }
    else                         { format!("{n} B")          }
}
...
pub fn print_header() {
    println!(
        "{:<12}{:<8}{:<14}{:<14}{:<14}{:<14}{:<14}",
        "BlkSize", "Batch", "BW (GB/S)", "Avg Lat (us)", "Avg Tx (us)", "P99 Tx (us)", "P999 Tx (us)",
    );
    println!("{RULE}");
}
...
pub fn print_table(rep: &Report) {
    print_header();
    for c in &rep.cells { print_row(c); }
}
