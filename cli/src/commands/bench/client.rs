...
use super::report::{print_header, print_preamble, print_row, print_table, Cell, ClientPreamble, Report};
use super::{parse_size, ClientArgs, OpArg};

pub async fn run(args: ClientArgs) -> Result<()> {
    let mode = resolve_mode(args.config.as_deref(), args.mode)?;
    let block_bytes: Vec<u64> = args.block_sizes.iter()
        .map(|s| parse_size(s))
        .collect::<Result<_>>()?;
...
async fn run_tls(args: &ClientArgs, block_bytes: &[u64]) -> Result<Report> {
    ...
    let resolved: Vec<std::net::SocketAddr> = std::net::ToSocketAddrs::to_socket_addrs(&args.target)
        .map(|i| i.collect())
        .unwrap_or_default();
    ...
    let probe = async {
        let resp = client.get(format!("{url_base}/bench/echo/4"))?.send().await?;
        ...
        for &batch in &args.batch_sizes {
            for &block in block_bytes {
                let cell = run_cell_tls(
                    &client, &url_base, block, batch, threads,
                    args.op, op_str, warmup, duration,
                ).await?;
                print_row(&cell);
                cells.push(cell);
            }
...
async fn run_cell_tls(
    ...
    if !warmup.is_zero() {
        drive(client, url_base, block, batch, threads, op, warmup, None, None, None).await?;
    }
    let started = Instant::now();
    drive(
        client, url_base, block, batch, threads, op, duration,
        Some(hist.clone()), Some(iters.clone()), Some(bytes.clone()),
    ).await?;
...
async fn drive(
    ...
    for _ in 0..threads {
        ...
        tasks.push(compio::runtime::spawn(async move {
            worker(client, url_base, block, batch, op, deadline, hist, iters, bytes).await
        }));
    }
    for t in tasks {
...
async fn one_call(
    ...
    match op {
        OpArg::Read => {
            ...
            let resp = client.get(url).context("build GET")?
                .send().await.context("GET send")?;
        ...
        OpArg::Write => {
            ...
            let resp = client.put(url).context("build PUT")?
                .body(body)
                .send().await.context("PUT send")?;
            let status = resp.status();
